#!/usr/bin/env python3
"""Generate solvable 4x4 sliding-tile benchmark instances.

Instances are generated by random walks from the goal state, so solvability is
guaranteed by construction. The output contains four cost-model clones for each
base layout; the domain file supplies the cost structure, and these problem
files supply the numeric tile costs.
"""
...
def position_names() -> list[str]:
    return [f"pos{col}{row}" for row in range(SIZE) for col in range(SIZE)]
...
def neighbors(blank_index: int) -> list[int]:
    ...
    result = []
    if row > 0:
        result.append(blank_index - SIZE)
    if row < SIZE - 1:
        result.append(blank_index + SIZE)
    if col > 0:
...
def cost_assignments(cost_model: str) -> list[str]:
    ...
    if cost_model == "unit":
        ...
        heavy_tiles = set(range(NUM_TILES - 3, NUM_TILES + 1))
        costs = {
...
def problem_text(instance_id: int, cost_model: str, board: tuple[int, ...]) -> str:
    problem_name = f"sliding-tile-4x4-{instance_id:03d}-{cost_model}"
...
def write_manifest(
    ...
    lines = [
        ...
        "# pos00 pos10 pos20 pos30 pos01 ... pos33",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="problems/sliding-tile/4x4")
    ...
    for cost_model in cost_models:
        ...
        for instance_id, board in enumerate(boards, start=1):
            problem_path = model_dir / f"sliding-tile-4x4-{instance_id:03d}.pddl"
