#include "alternating_wastar_focal_gbfs.h"
...
namespace alternating_wastar_focal_gbfs {
...
AlternatingWAStarFocalGBFS::AlternatingWAStarFocalGBFS(
...
void AlternatingWAStarFocalGBFS::initialize() {
    log << "Conducting alternating WA*/focal-GBFS search"
...
bool AlternatingWAStarFocalGBFS::compute_entry_values(
...
bool AlternatingWAStarFocalGBFS::is_valid_entry(const Entry &entry) {
    State state = state_registry.lookup_state(entry.state_id);
    SearchNode node = search_space.get_node(state);
    return node.is_open() && node.get_g() == entry.g;
}

void AlternatingWAStarFocalGBFS::prune_bucket(
...
optional<int> AlternatingWAStarFocalGBFS::get_min_live_key(
...
void AlternatingWAStarFocalGBFS::insert_open_entry(const Entry &entry) {
    open_by_f[entry.f].push_back(entry);
    open_by_weighted_f[entry.weighted_f].push_back(entry);
}

optional<AlternatingWAStarFocalGBFS::Entry>
AlternatingWAStarFocalGBFS::select_wastar_entry() {
...
optional<AlternatingWAStarFocalGBFS::Entry>
AlternatingWAStarFocalGBFS::select_focal_gbfs_entry() {
    ...
    while (it != open_by_f.end() && static_cast<double>(it->first) <= threshold) {
        ...
        for (const Entry &entry : it->second) {
            if (entry.h < best_h) {
                best_h = entry.h;
                num_best_h_entries = 1;
                selected = entry;
            } else if (entry.h == best_h) {
                ++num_best_h_entries;
                uniform_int_distribution<int> dist(1, num_best_h_entries);
                if (dist(rng) == 1) {
                    selected = entry;
                }
            }
...
optional<SearchNode> AlternatingWAStarFocalGBFS::get_next_node_to_expand() {
    while (true) {
        optional<Entry> selected = use_wastar_next ? select_wastar_entry()
                                                  : select_focal_gbfs_entry();
        if (!selected) {
            selected = use_wastar_next ? select_focal_gbfs_entry()
                                      : select_wastar_entry();
        }
        if (!selected) {
            return nullopt;
...
SearchStatus AlternatingWAStarFocalGBFS::step() {
...
SearchStatus AlternatingWAStarFocalGBFS::expand(const SearchNode &node) {
...
void AlternatingWAStarFocalGBFS::generate_successors(const SearchNode &node) {
    const State &state = node.get_state();
    ...
    for (OperatorID op_id : applicable_operators) {
        ...
        if (succ_node.is_new()) {
            ...
            insert_open_entry(Entry(succ_state.get_id(), succ_g, h, f, weighted_f));
...
void AlternatingWAStarFocalGBFS::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
    pruning_method->print_statistics();
}

class AlternatingWAStarFocalGBFSFeature
    : public plugins::TypedFeature<SearchAlgorithm, AlternatingWAStarFocalGBFS> {
public:
    AlternatingWAStarFocalGBFSFeature()
        : TypedFeature("alternating_wastar_focal_gbfs") {
        document_title("Alternating WA*/focal GBFS search");
        document_synopsis(
            "Alternates between WA* expansion and focal-list GBFS expansion. "
            "The focal mode restricts to nodes with f <= focal_w * f_min and "
            "chooses a random node among those with minimum h.");
        ...
        add_option<int>("random_seed", "seed for random focal tie breaking", "0");
        add_search_pruning_options_to_feature(*this);
        add_search_algorithm_options_to_feature(
            *this, "alternating_wastar_focal_gbfs");
    }

    virtual shared_ptr<AlternatingWAStarFocalGBFS> create_component(
        const plugins::Options &opts) const override {
        return plugins::make_shared_from_arg_tuples<AlternatingWAStarFocalGBFS>(
...
static plugins::FeaturePlugin<AlternatingWAStarFocalGBFSFeature> _plugin;
}
