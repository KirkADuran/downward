...
namespace plugin_my_astar {
class MyAStarSearchFeature
    : public plugins::TypedFeature<SearchAlgorithm, eager_search::EagerSearch> {
public:
    MyAStarSearchFeature() : TypedFeature("my_astar") {
        document_title("My A* search (eager)");
        document_synopsis(
            "My A* is a special case of eager best first search that uses g+h "
            "as f-function. "
        ...
        eager_search::add_eager_search_options_to_feature(*this, "my_astar");
...
static plugins::FeaturePlugin<MyAStarSearchFeature> _plugin;
}
